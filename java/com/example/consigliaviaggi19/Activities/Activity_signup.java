...
public class Activity_signup extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        campo_nickname = findViewById(R.id.editTextUsername);
    ...
    public void iscrivitiIsClicked(View view) {
        ...
        } else if (!Controller.isValidName(nickname)){
            Toast.makeText(getApplicationContext(), "Inserire username", Toast.LENGTH_SHORT).show();
        }else{
            if (flag){
                bool = Controller.createUser(email, password,nome,cognome,nickname,1);
            }else{
                bool = Controller.createUser(email, password,nome,cognome,nickname,0);
            }
            if (bool == true){
